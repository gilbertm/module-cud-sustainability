/**
 * @file
 * Drupal behavior for CUD Research carousels.
 *
  ...
  Drupal.behaviors.cudResearchNavigation = {
    attach: function (context) {
      ...
      navContainers.forEach(function (navContainer) {
        ...
        var toggleBtn = navContainer.querySelector('[data-research-nav-toggle]');
        var navPanels = navContainer.querySelector('[data-research-nav-panels]');
        if (!toggleBtn || !navPanels) return;

        navContainer.dataset.cudNavInit = '1';
        var daisyDropdownItems = navContainer.querySelectorAll('.research-primary-item.dropdown');

        function closeAllDaisyDropdowns(exceptItem) {
          daisyDropdownItems.forEach(function (item) {
            if (item !== exceptItem) {
              item.classList.remove('is-dropdown-open');
              var trigger = item.querySelector('.research-primary-item__link');
              if (trigger) {
        ...
        navContainer.querySelectorAll('[data-research-submenu]').forEach(function (item) {
          var panel = item.querySelector('[data-research-submenu-panel]');
          if (panel) {
        ...
        function ensurePanelPortal() {
          var portal = document.querySelector('[data-research-mega-portal]');
          if (!portal) {
            portal = document.createElement('div');
            portal.setAttribute('data-research-mega-portal', '');
            portal.className = 'research-mega-panel-portal';
      ...
      function repositionOpenDetachedPanel() {
        ...
        var openItem = navContainer.querySelector('[data-research-submenu].is-submenu-open');
        if (!openItem) {
          return;
        }
      ...
      function setItemOpenState(item, isOpen) {
        var itemToggle = item.querySelector('[data-research-submenu-toggle]');
      ...
      function closeAllSubmenus(exceptItem) {
        navContainer.querySelectorAll('[data-research-submenu].is-submenu-open').forEach(function (openedItem) {
      ...
      navContainer.querySelectorAll('[data-research-submenu-toggle]').forEach(function (itemToggle) {
        itemToggle.addEventListener('click', function (event) {
          var item = itemToggle.closest('[data-research-submenu]');
      ...
      navContainer.querySelectorAll('[data-research-submenu] .research-primary-item__link').forEach(function (parentLink) {
        parentLink.addEventListener('click', function (event) {
          if (!window.matchMedia('(min-width: 1024px)').matches) {
            return;
          }

          var item = parentLink.closest('[data-research-submenu]');
          if (!item) {
            return;
          }
      ...
      navContainer.addEventListener('click', function (event) {
        var itemToggle = event.target.closest('[data-research-submenu-toggle]');
