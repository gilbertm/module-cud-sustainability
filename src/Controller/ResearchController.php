...
namespace Drupal\cud_research\Controller;
...
/**
 * Handles CUD research routes under /research.
 */
class ResearchController extends ControllerBase {

  /**
   * Research content type machine name.
   */
  protected const RESEARCH_BUNDLE = 'research';
  ...
  /**
   * Constructs a ResearchController object.
   */
  public function __construct(EntityTypeManagerInterface $entity_type_manager, EntityFieldManagerInterface $entity_field_manager, RequestStack $request_stack, MenuLinkTreeInterface $menu_link_tree, LoggerChannelFactoryInterface $logger_factory) {
    ...
    $this->logger = $logger_factory->get('cud_research');
  }
  ...
  public function index(): array {
    ...
    if ($main_node) {
      ...
      $build = $this->buildPage(
        ...
        $this->getMenuItems('cud-research-primary'),
        $this->getMenuItems('cud-research-secondary')
      );
    ...
    return $this->buildPage(
      $this->t('Research Overview')->render(),
      [
        '#markup' => $this->t('Welcome to the Research Hub.'),
      ],
      $mode,
      [],
      [],
  ...
  /**
   * Displays research-only search results.
   */
  ...
  /**
   * Renders a research node from a slug.
   */
  ...
  /**
   * Loads a published research node by configured slug value.
   */
  ...
  * @return list<array{image: array|null, image_url: string, overlay: array|null}>
  ...
  /**
   * Returns published research nodes matching a free-text query.
   *
   * @return list<\Drupal\node\NodeInterface>
   *   Matching research nodes.
   */
  ...
  protected function buildSearchResultsContent(string $query, array $results, string $mode): array {
    ...
    if ($query === '') {
      $content['summary'] = [
        '#type' => 'html_tag',
        '#tag' => 'p',
        '#value' => (string) $this->t('Enter a keyword in the search field above to search CUD research content.'),
        '#attributes' => [
    ...
    if ($results === []) {
      $content['empty'] = [
        '#type' => 'html_tag',
        '#tag' => 'p',
        '#value' => (string) $this->t('No research pages matched your search.'),
        '#attributes' => [
          'class' => ['text-sm', 'leading-7', 'text-slate-600'],
        ],
      ];
  ...
  /**
   * Builds a search-results link for a research node.
   */
  protected function buildSearchResultUrl(NodeInterface $node, string $mode): Url {
    ...
    if ($slug !== '') {
      return Url::fromRoute('cud_research.sub_pages', ['slug' => $slug], ['query' => $query]);
    }
  ...
  /**
   * Returns the configured research slug for a node.
   */
  ...
  protected function buildSearchExcerpt(NodeInterface $node, string $query): string {
    ...
    if ($text === '') {
      return (string) $this->t('Open this research page to view the full content.');
    }
  ...
  /**
   * Builds the common research page shell.
   */
  ...
  protected function resolveThemeHook(string $mode): string {
    return $mode === 'secondary' ? 'cud_research_secondary' : 'cud_research_main';
  }
